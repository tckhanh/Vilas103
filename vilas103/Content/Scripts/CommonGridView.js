(function () {
    ...
    function OnContextMenu(s, e) {
        ...
        if (e.objectType == "row" || e.objectType == "emptyrow") {
            ...
            if (myIsAuthenticated == true && document.getElementById("LeftPanel_LeftPanelContent_ASPxNavBar1") != undefined) {
    ...
    function endCallback(s, e) {
        switch (e.command) {
            case 'CUSTOMCALLBACK':
                ...
                //if (s.cpParameters.substring(0, 7) == myConstant.Tokens_Callback) {
                //    var sumPrice = s.cpParameters.substring(7);
                //    gridView.SetEditValue("Price", sumPrice);
                //}
                break;
            //case 'REFRESH':
            //    updateNavBarName();
    ...
    function updateNavBarName() {
        ...
        if (document.getElementById("LeftPanel_LeftPanelContent_ASPxNavBar1") != undefined) {
            ...
            if (gridView.cpUpdatedByCount != undefined) {
                NavBarItem = filtersNavBar.GetItemByName("UpdatedBy");
                NavBarItemTile = filtersNavBar.GetItemByName("UpdatedBy").GetText();
                NavBarItemTile = NavBarItemTile.split('(')[0];
                if (gridView.cpUpdatedByCount > 0) {
                    NavBarItem.SetText(NavBarItemTile + " (" + gridView.cpUpdatedByCount + ")");
                }
                else {
                    NavBarItem.SetText(NavBarItemTile);
                }
