...
{
    public partial class MailMergeRequest : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsCallback && Request["Template"] != null && Request["ID"] != null)
                ...
                IEnumerable<RequestVM> items = GetData(ID);
        ...
        IEnumerable<RequestVM> GetData(string ID)
        {
            RequestVM item = Mapper.Map<RequestVM>(RequestProvider.GetSingleByCondition(x=> x.Id.ToString() == ID, new string[] { "Company" }));
            List<RequestVM> ItemList = new List<RequestVM>();
            if (item != null)
            {
                ItemList.Add(item);
            }
