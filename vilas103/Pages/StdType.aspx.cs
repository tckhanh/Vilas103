...
namespace vilas103
{
    public partial class StdType : System.Web.UI.Page
    {
        ...
        {
            StdTypeVM CloneItem = new StdTypeVM();
            if (bool.Parse(HttpContext.Current.Session["isCloneRow_Com"].ToString()) == true && GridView.FocusedRowIndex >= 0)
            {
                ...
                CloneItem = Mapper.Map<StdTypeVM>(StdTypeProvider.GetSingleById(GridView.GetRowValues(GridView.FocusedRowIndex, "Id").ToString()));


                HttpContext.Current.Session["isCloneRow_Com"] = false;

                PropertyInfo myFieldInfo;
                foreach (var myProperty in typeof(StdTypeVM).GetProperties())
                {
                    myFieldInfo = typeof(StdTypeVM).GetProperty(myProperty.Name);
            ...
            switch (e.Parameters)
            {
                ...
                case "Actived":
                    if (GridView.FocusedRowIndex >= 0)
                    {
                        ...
                        StdTypeProvider.SetActived(itemID, true);
                        GridView.DataBind();
                        UpdateNavBarName();
                    }
                    GridView.JSProperties["cpMessage"] = string.Format("OrderDate {0} is later than {1}", DateTime.Now.ToShortDateString(), DateTime.Now.ToShortDateString());
                ...
                case "InActived":
                    ...
                    {
                        var itemID = GridView.GetRowValues(GridView.FocusedRowIndex, "Id").ToString();
                        StdTypeProvider.SetActived(itemID, false);
                        GridView.DataBind();
                        UpdateNavBarName();
                    }
                    GridView.JSProperties["cpMessage"] = string.Format("OrderDate {0} is later than {1}", DateTime.Now.ToShortDateString(), DateTime.Now.ToShortDateString());
                ...
                case "Blocked":
                    ...
                    {
                        var itemID = GridView.GetRowValues(GridView.FocusedRowIndex, "Id").ToString();
                        StdTypeProvider.SetBlocked(itemID, true, User.Identity.Name);
                        GridView.DataBind();
                        UpdateNavBarName();
                    }
                    GridView.JSProperties["cpMessage"] = string.Format("OrderDate {0} is later than {1}", DateTime.Now.ToShortDateString(), DateTime.Now.ToShortDateString());
                ...
                case "UnBlocked":
                    ...
                    {
                        var itemID = GridView.GetRowValues(GridView.FocusedRowIndex, "Id").ToString();
                        StdTypeProvider.SetBlocked(itemID, false, User.Identity.Name);
                        GridView.DataBind();
                        UpdateNavBarName();
                    }
                    GridView.JSProperties["cpMessage"] = string.Format("OrderDate {0} is later than {1}", DateTime.Now.ToShortDateString(), DateTime.Now.ToShortDateString());
                ...
                case "InActivedBlocked":
                    ...
                    {
                        var itemID = GridView.GetRowValues(GridView.FocusedRowIndex, "Id").ToString();
                        StdTypeProvider.SetActivedBlocked(itemID, false, true, User.Identity.Name);
                        GridView.DataBind();
                        UpdateNavBarName();
                    }
                    GridView.JSProperties["cpMessage"] = string.Format("OrderDate {0} is later than {1}", DateTime.Now.ToShortDateString(), DateTime.Now.ToShortDateString());
                ...
                case "ActivedUnBlocked":
                    ...
                    {
                        var itemID = GridView.GetRowValues(GridView.FocusedRowIndex, "Id").ToString();
                        StdTypeProvider.SetActivedBlocked(itemID, true, false, User.Identity.Name);
                        GridView.DataBind();
                        UpdateNavBarName();
                    }
                    GridView.JSProperties["cpMessage"] = string.Format("OrderDate {0} is later than {1}", DateTime.Now.ToShortDateString(), DateTime.Now.ToShortDateString());
        ...
        protected void GridView_RowValidating(object sender, DevExpress.Web.Data.ASPxDataValidationEventArgs e)
        {
            ...
            if (GridView.IsNewRowEditing)
            {
                ...
                foreach (GridViewColumn column in GridView.Columns)
                {
                    ...
                    PropertyInfo myFieldInfo = typeof(StdTypeVM).GetProperty(dataColumn.FieldName);
                    if (myFieldInfo == null) continue;
        ...
        private void UpdateNavBarName()
        {
            ...
            GridView.JSProperties["cpAllCount"] = StdTypeProvider.Count(x => true);
            GridView.JSProperties["cpInActivedUnBlockedCount"] = StdTypeProvider.Count(x => x.Actived == false && x.Blocked == false);
            GridView.JSProperties["cpActivedBlockedCount"] = StdTypeProvider.Count(x => x.Actived == true && x.Blocked == true);
            GridView.JSProperties["cpInactivedBlockedCount"] = StdTypeProvider.Count(x => x.Actived == false && x.Blocked == true);
            GridView.JSProperties["cpActivedUnBlockedCount"] = StdTypeProvider.Count(x => x.Actived == true && x.Blocked == false);
            GridView.JSProperties["cpUpdatedByCount"] = StdTypeProvider.Count(x => x.CreatedBy == User.Identity.Name || x.UpdatedBy == User.Identity.Name);
        }
    }
}