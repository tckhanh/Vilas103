...
namespace vilas103
{
    public partial class EquGroup : System.Web.UI.Page
    {
        ...
        {
            EquGroupVM CloneItem = new EquGroupVM();
            if (bool.Parse(HttpContext.Current.Session["isCloneRow_Com"].ToString()) == true && GridView.FocusedRowIndex >= 0)
            {
                ...
                //CloneItem = EquGroupProvider.GetSingleById(selectedIds.Last());

                CloneItem = Mapper.Map<EquGroupVM>(EquGroupProvider.GetSingleById(GridView.GetRowValues(GridView.FocusedRowIndex, "Id").ToString()));


                HttpContext.Current.Session["isCloneRow_Com"] = false;

                PropertyInfo myFieldInfo;
                foreach (var myProperty in typeof(EquGroupVM).GetProperties())
                {
                    myFieldInfo = typeof(EquGroupVM).GetProperty(myProperty.Name);
            ...
            switch (e.Parameters)
            {
                ...
                case "Actived":
                    if (GridView.FocusedRowIndex >= 0)
                    {
                        ...
                        EquGroupProvider.SetActived(itemID, true);
                        GridView.DataBind();
                        UpdateNavBarName();
                    }
                    GridView.JSProperties["cpMessage"] = string.Format("OrderDate {0} is later than {1}", DateTime.Now.ToShortDateString(), DateTime.Now.ToShortDateString());
                ...
                case "InActived":
                    ...
                    {
                        var itemID = GridView.GetRowValues(GridView.FocusedRowIndex, "Id").ToString();
                        EquGroupProvider.SetActived(itemID, false);
                        GridView.DataBind();
                        UpdateNavBarName();
                    }
                    GridView.JSProperties["cpMessage"] = string.Format("OrderDate {0} is later than {1}", DateTime.Now.ToShortDateString(), DateTime.Now.ToShortDateString());
                ...
                case "Blocked":
                    ...
                    {
                        var itemID = GridView.GetRowValues(GridView.FocusedRowIndex, "Id").ToString();
                        EquGroupProvider.SetBlocked(itemID, true, User.Identity.Name);
                        GridView.DataBind();
                        UpdateNavBarName();
                    }
                    GridView.JSProperties["cpMessage"] = string.Format("OrderDate {0} is later than {1}", DateTime.Now.ToShortDateString(), DateTime.Now.ToShortDateString());
                ...
                case "UnBlocked":
                    ...
                    {
                        var itemID = GridView.GetRowValues(GridView.FocusedRowIndex, "Id").ToString();
                        EquGroupProvider.SetBlocked(itemID, false, User.Identity.Name);
                        GridView.DataBind();
                        UpdateNavBarName();
                    }
                    GridView.JSProperties["cpMessage"] = string.Format("OrderDate {0} is later than {1}", DateTime.Now.ToShortDateString(), DateTime.Now.ToShortDateString());
                ...
                case "InActivedBlocked":
                    ...
                    {
                        var itemID = GridView.GetRowValues(GridView.FocusedRowIndex, "Id").ToString();
                        EquGroupProvider.SetActivedBlocked(itemID, false, true, User.Identity.Name);
                        GridView.DataBind();
                        UpdateNavBarName();
                    }
                    GridView.JSProperties["cpMessage"] = string.Format("OrderDate {0} is later than {1}", DateTime.Now.ToShortDateString(), DateTime.Now.ToShortDateString());
                ...
                case "ActivedUnBlocked":
                    ...
                    {
                        var itemID = GridView.GetRowValues(GridView.FocusedRowIndex, "Id").ToString();
                        EquGroupProvider.SetActivedBlocked(itemID, true, false, User.Identity.Name);
                        GridView.DataBind();
                        UpdateNavBarName();
                    }
                    GridView.JSProperties["cpMessage"] = string.Format("OrderDate {0} is later than {1}", DateTime.Now.ToShortDateString(), DateTime.Now.ToShortDateString());
                ...
                case "Delete":
                    ...
                    {
                        var itemID = GridView.GetRowValues(GridView.FocusedRowIndex, "Id").ToString();
                        EquGroupProvider.Delete(itemID);
                        //EquGroupProvider.Delete(itemID);
                        GridView.DataBind();
                        UpdateNavBarName();
                    }
                    ...
                    //{
                    //    EquGroupProvider.DeleteSuit(item, false);
                    //    HttpContext.Current.Session["isActBloChanged_Com"] = true;
                    //}
                    //GridView.DataBind();
                    //GridView.JSProperties["cpMessage"] = string.Format("OrderDate {0} is later than {1}", DateTime.Now.ToShortDateString(), DateTime.Now.ToShortDateString());
        ...
        {
            EquGroupVM item = new EquGroupVM();
            if (GridView.IsNewRowEditing)
            {
                ...
                foreach (GridViewColumn column in GridView.Columns)
                {
                    ...
                    PropertyInfo myFieldInfo = typeof(EquGroupVM).GetProperty(dataColumn.FieldName);
                    if (myFieldInfo == null) continue;
        ...
        protected void GridView_EditFormLayoutCreated(object sender, ASPxGridViewEditFormLayoutEventArgs e)
        {
            ...
            //    if (dataColumn != null)
            //    {
            //        PropertyInfo MyPropertyInfo = typeof(EquGroupVM).GetProperty(dataColumn.FieldName);
        ...
        protected void GridView_CustomJSProperties(object sender, ASPxGridViewClientJSPropertiesEventArgs e)
        {
            ...
            //    e.Properties["cpAllCount"] = EquGroupProvider.Count(x => true);
            //    e.Properties["cpInActivedUnBlockedCount"] = EquGroupProvider.Count(x => x.Actived == false && x.Blocked == false);
            //    e.Properties["cpActivedBlockedCount"] = EquGroupProvider.Count(x => x.Actived == true && x.Blocked == true);
            //    e.Properties["cpInactivedBlockedCount"] = EquGroupProvider.Count(x => x.Actived == false && x.Blocked == true);
            //    e.Properties["cpActivedUnBlockedCount"] = EquGroupProvider.Count(x => x.Actived == true && x.Blocked == false);
            //    e.Properties["cpOwnerCount"] = EquGroupProvider.Count(x => x.UserName == User.Identity.Name);
            //}
        }
        ...
        private void UpdateNavBarName()
        {
            ...
            GridView.JSProperties["cpAllCount"] = EquGroupProvider.Count(x => true);
            GridView.JSProperties["cpInActivedUnBlockedCount"] = EquGroupProvider.Count(x => x.Actived == false && x.Blocked == false);
            GridView.JSProperties["cpActivedBlockedCount"] = EquGroupProvider.Count(x => x.Actived == true && x.Blocked == true);
            GridView.JSProperties["cpInactivedBlockedCount"] = EquGroupProvider.Count(x => x.Actived == false && x.Blocked == true);
            GridView.JSProperties["cpActivedUnBlockedCount"] = EquGroupProvider.Count(x => x.Actived == true && x.Blocked == false);
            GridView.JSProperties["cpUpdatedByCount"] = EquGroupProvider.Count(x => x.CreatedBy == User.Identity.Name || x.UpdatedBy == User.Identity.Name);
        }
    }
}